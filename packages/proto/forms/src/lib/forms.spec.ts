...
describe('FieldResolver interact sync', () => {
  it('should sync disabled state from field state to Interact directive', async () => {
    @Component({
      selector: 'test-disabled-sync',
      changeDetection: ChangeDetectionStrategy.OnPush,
      imports: [FormRoot, FormField, ProtoFormField],
      template: `
        <form [formRoot]="form">
    ...
    class DisabledHost {
      readonly model = signal({ name: 'James', isDisabled: true });
      readonly form = form(
        this.model,
        path => {
          resolver(path.name, Interact, ctx => {
            ctx.instance.disabled.set(ctx.stateOf(path.isDisabled).value());
          });
        },
        noopSubmission,
      );
    ...
    fixture.componentInstance.model.update(m => ({ ...m, isDisabled: false }));
    fixture.detectChanges();
